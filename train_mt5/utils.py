...
def convert_alpaca_to_conversations(sample, idx):
    # split text components
    text = sample["text"]
    values = text.split("###")
    system_prompt = values[0].strip()
    instruction = values[1].replace("Instruction:", "").strip()
    response = values[-1].replace("Response:", "").strip()
    if len(values) != 3:
        instruction = instruction + "\n" + values[2].replace("Input:", "").strip()

    conv = f"{system_prompt}\nUSER: {instruction}\nASSISTANT:{response}"
    sample["text"] = conv
    sample["id"] = idx
    return sample
